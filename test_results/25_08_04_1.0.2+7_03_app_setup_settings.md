...
| Name required error shows when empty | No error |
| Name accepts letters & spaces | No limit |
| Name rejects numbers/symbols | No limit |
| Name input capped at 50 chars | No limit |
| *Title* field present | ✅ |
| Title input capped at 100 chars | No limit |
| *Short Bio* field present | ✅ |
| Short Bio input capped at 300 chars | No limit |
| LinkedIn URL field present | ✅ |
| LinkedIn URL must start with http(s):// | No limit |
| Instagram URL field present | ✅ |
| Instagram URL must start with http(s):// | No limit |
| Facebook URL field present | ✅ |
| Facebook URL must start with http(s):// | No limit |
| TikTok URL field present | ✅ |
| TikTok URL must start with http(s):// | No limit |
| Discord URL field present | ✅ |
| Discord URL must start with http(s):// | No limit |
| Website URL field present | ✅ |
| Website URL must start with http(s):// | No limit |
| “Save Changes” button disabled when form invalid | Always active |
| “Save Changes” enabled when form valid | ✅ |
| Tap Save shows toast “Profile updated” | No toast |
...
| “Send Link” button disabled until email valid | Active but gives error on miss|
| Tap Send shows “Password reset email sent” | ✅ |
| **Edit Contact Info** opens modal | ✅ |
| Email field present in Contact Info | ✅ |
| Email required validation works | Can leave blank |
| Email input capped at 254 chars | No limit |
| Phone field present | ✅ |
| Phone accepts digits & “+” only | No limit|
| Phone rejects letters | No limit |
| Phone input capped at 20 chars | No limit |
| Social icon description field present | ✅ |
| Description capped at 100 chars | No limit |
| “Save” button disabled until required valid | Always active|
| Tap Save closes modal | ✅ |
| Re-opening modal shows saved values | ✅ |
| **Tutorial Page** item navigates to onboarding tutorial | ✅ |
| Back navigation from tutorial returns to My Settings | Navigates to dashboard |
| **Submit a Bug** opens Bug Report form | ✅ |
| Bug Name field present | ✅ |
| Bug Name required validation works | ✅ |
| Bug Name capped at 100 chars | No limit |
| Bug Short Description field present | ✅ |
| Bug Short Description required validation works | ✅ |
| Bug Short Description capped at 500 chars | No limit |
| Urgency selector shows default value 1 | ✅ |
| Screenshot/video upload field present | ✅ |
| Submit Ticket disabled until form valid | Active but gives error on miss|
| Tap Submit shows toast “Ticket submitted” | ✅ |
| After toast, returns to My Settings | Returns to dashboard |
| **Submit a Feature Request** opens Feature form | ✅ |
| Feature Name field present | ✅ |
| Feature Name required validation works | ✅ |
| Feature Name capped at 100 chars | No limit |
| Feature Short Description required validation works | ✅ |
| Feature Short Description capped at 500 chars | No limit |
