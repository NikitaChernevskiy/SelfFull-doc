...
| Splash screen hides ≤ 3 s | ❌ **Issue location:** App launch splash screen (iOS). **Where:** Immediately after tapping SelfFull icon. **Details:** Splash remains visible for ~5 seconds instead of the required ≤ 3 seconds, causing a slower-than-expected startup experience. |
...
| Name rejects numbers/symbols | ❌ **Issue location:** Create Account form → Name field. **Where:** Sign-up screen. **Details:** Field currently allows numbers and special characters even though only letters and spaces are permitted by requirements. |
...
| Email input capped at 254 chars | ❌ **Issue location:** Create Account form → Email field. **Where:** Sign-up screen. **Details:** No enforced limit or visible error when exceeding 254 characters; field continues accepting additional input. |
| **Password** field is present | ✅ |
| Password field shows error when empty | ✅ |
| Password min length 8 enforced | ❌ **Issue location:** Create Account form → Password field. **Where:** Sign-up screen. **Details:** Minimum length requirement incorrectly set to 6 instead of 8 characters. |
| Password must include 1 uppercase | ❌ **Issue location:** Create Account form → Password field. **Where:** Sign-up screen. **Details:** Field does not enforce presence of an uppercase letter; accepts passwords without it. |
| Password must include 1 number | ❌ **Issue location:** Create Account form → Password field. **Where:** Sign-up screen. **Details:** Field does not enforce presence of a numeric digit. |
| Password must include 1 symbol | ❌ **Issue location:** Create Account form → Password field. **Where:** Sign-up screen. **Details:** Field does not enforce presence of a special character. |
