...
| **Play Store** search “SelfFull” returns app within top 5 | ❌ **Issue location:** Google Play Store app (Android device). **Where:** Search results screen after typing `SelfFull`. **Details:** The app does not appear in the top 5 results; appears lower in the list or not at all unless searching with a different keyword such as `"Self Full"`. This causes discoverability problems for new users searching the exact app name without space. |
...
| Store page shows correct screenshots (App Store) | ❌ **Issue location:** Apple App Store app (iOS device). **Where:** SelfFull store listing page in the screenshots carousel. **Details:** Screenshots displayed are outdated — they do not match the current UI/UX of the installed version. This may confuse users and misrepresent the current product features. |
| Store page shows correct screenshots (Play Store) | ❌ **Issue location:** Google Play Store app (Android device). **Where:** SelfFull store listing page in the screenshots section. **Details:** Screenshots are from an older app version and do not reflect the current interface or features, causing inconsistencies with what users see after installation. |
